...
(function() {
  ...
  if (typeof window !== 'undefined') {
    // Function to initialize preview templates
    function initPreviewTemplates() {
      // Try to get CMS - Decap CMS v3 uses window.CMS
      const CMS = window.CMS;
      
      if (!CMS) {
        // Retry if CMS not loaded yet
        setTimeout(initPreviewTemplates, 100);
        return;
      }
      ...
      try {
        ...
        console.error('Could not get React from CMS:', e);
        return;
      }
      ...
      // Helper to render markdown-like text as React elements
      function renderMarkdown(text) {
        ...
        while (remaining.includes('**')) {
          ...
          const afterBold = remaining.substring(boldStart + 2);
          const boldEnd = afterBold.indexOf('**');
          if (boldEnd !== -1) {
            const boldText = afterBold.substring(0, boldEnd);
            parts.push(h('strong', { key: key++ }, boldText));
            remaining = afterBold.substring(boldEnd + 2);
          } else {
            parts.push('**' + afterBold);
            break;
          }
        }
        ...
        // Return array of text and React elements
      ...
      function PieCardPreview({ entry, widgetFor }) {
        ...
        return h('div', {
          ...
          h('div', {
            ...
            }, renderMarkdown(description)) : null,
            shortDescription ? h('p', { 
              style: { margin: '0 0 10px 0', color: '#555' } 
            }, renderMarkdown(shortDescription)) : null,
            ingredients ? h('p', { 
              style: { margin: '10px 0', color: '#666', fontSize: '0.9rem' } 
            }, renderMarkdown(ingredients)) : null,
            isDinnerPie && (smallSoldOut || bigSoldOut) && !(smallSoldOut && bigSoldOut) ? [
              ...
              }, renderMarkdown(smallSoldOutComment)) : null,
              bigSoldOut && bigSoldOutComment ? h('p', { 
                key: 'big', 
                style: { color: 'red', margin: '5px 0' } 
              }, renderMarkdown(bigSoldOutComment)) : null
            ] : null
          ])
        ]);
      }
      ...
      try {
        // Register templates - collection names must match config.yml exactly
        if (CMS.registerPreviewTemplate) {
          ...
          console.log('✅ Preview templates registered successfully for: fruit_pies, cream_pies, special_pies, dinner_pies, hand_pies');
        } else {
          console.error('❌ CMS.registerPreviewTemplate not available. CMS object:', CMS);
    ...
    // Start initialization - try multiple times to ensure CMS is loaded
    function startInit() {
      if (window.CMS) {
        console.log('CMS found, initializing preview templates...');
